...
\begin{document}
	...
	\begin{komplexaufgaben}{sec:kurvenDissLog}		
		...
		\komplexaufgabe{Gegeben ist die Kurvenschar $f_a(x)=x\cdot (a-\ln x), a>0,x>0.$}{
			...
			\item Berechnen Sie den Inhalt der Fläche, die im ersten Quadranten vom Graphen von $f_1$, der $x$-Achse und der Geraden 
			durch den Hochpunkt von $f_1$ umschlossen wird.
	...
	\begin{komplexloesungen}{sec:kurvenDissLog}
		...
		\komplexloesung{}{
			\item $f_a'(x)=a-\ln x -1$, $f_a''(x)=-\frac{1}{x}$ 
			 ...
			 \end{tikzpicture}
			 \end{minipage}
			 \item $F_a(x)=\frac{x^2}{2}(a-\ln x +\frac{1}{2}) +C$
